...
import rich.box
from typing import List, Optional, Union, cast, Dict, Tuple, Any
...
class VerboseAdapter(logging.LoggerAdapter):
    """
    A logger adapter that discards the message if the `verbose` flag
    is set to false.
    ...
    class A:
        def __init__(self, verbose=True):
            logger = logging.getLogger(__name__)
            self.logger = VerboseAdapter(logger, dict(verbose=verbose))
            self.logger.info(...)
            self.logger.error(...)
    ```
    """
...
class PatchJob:
    """
    Holds the data related to an LLM patch generation job,
    as opposed to `RunningJob`, which holds the data
    related to an evaluation job.

    A 'patch' is a valid mutation of the parent program generated by an LLM.
    - For a 'full' patch, the LLM is asked to generate the entire evolved block.
    - For a 'diff' patch, the LLM is asked to generate targeted
    modifications to the evolved block in a 'diff' format.
    """

    class PatchError(Exception):
        """
        Internal class to represent errors specific to this job.
        """
    ...
    def __init__(
        ...
        meta_recommendations: Tuple[Optional[str], Optional[str], Optional[str]],
    ):
        """
        Initializes this patch job.

        :param runner: the evolution runner
        :type runner: TunaEvolutionRunner
        :param current_gen: the current generation
        :type current_gen: int
        :param branch_id: int
        """
        if current_gen > runner.evo_config.num_generations:
            raise ValueError("error: current generation exceeds maximum number of generations")
        ...
        # Initialize prompt information
        self.parent_program = parent_program
        self.archive_programs = archive_programs
        self.top_k_programs = top_k_programs
        self.system_msg = system_msg
        self.user_msg = user_msg
        self.patch_type = patch_type
        if patch_type is None:
            assert current_gen == 0 and parent_program is None, "only generation 0 is allowed to have no patch type"
        elif patch_type in ["full", "cross"]:
            self.apply_patch = apply_full_patch
        elif patch_type == "diff":
            self.apply_patch = apply_diff_patch
        elif patch_type == "paper":
            raise NotImplementedError("Paper edit not implemented.")
            # self.apply_patch = apply_paper_patch
        else:
            raise ValueError(f"Invalid patch type: {patch_type}")
        self.msg_history: List[Dict] = []

        self.max_patch_attempts = runner.evo_config.max_patch_attempts
        self.llm_kwargs: Dict = runner.llm.get_kwargs()

        # Initialize outputs
        self.code_diff: Optional[str] = None
        self.api_cost: float = 0
        self.embed_cost: float = 0
        self.code_embedding: List[float] = []
        self.novelty_cost: float = 0
        self.num_novelty_checks: int = 0
        self.novelty_explanation: str = ""

        # Initialize metadata
        self.meta_patch_data: Dict[Any, Any] = {}
        meta_recs, meta_summary, meta_scratch = meta_recommendations
        # Add meta-recommendations/summary/scratchpad to meta_patch_data
        if meta_recs is not None:
            self.meta_patch_data["meta_recommendations"] = meta_recs
            self.meta_patch_data["meta_summary"] = meta_summary
    ...
    async def run(self) -> None:
        ...
        if self.current_gen == 0:
            # no patch can be generated without a parent -> submit directly
            self._submit_as_running_job()
            return
        ...
        if self.num_novelty_checks > 0:
            self.meta_patch_data["novelty_checks_performed"] = self.num_novelty_checks
            self.meta_patch_data["novelty_cost"] = self.novelty_cost
            self.meta_patch_data["novelty_explanation"] = self.novelty_explanation
        
        self._submit_as_running_job()

    async def _generate_patch(self, novelty_attempt_index: int) -> None:
        """
        Generates a valid patch for the parent program with retries.
        Note: this function will reset most entries of `self.meta_patch_data`
        to overwrite data from previous novelty attempts.

        :param novelty_attempt: the index of the novelty attempt
        :type novelty_attempt: int
        """

        assert self.parent_program is not None and self.current_gen != 0, "expected this job to have a parent program"

        # Notify the selection algorithm of the model being used
        if self.runner.llm_selection is not None:
            model_name = self.llm_kwargs["model_name"]
            self.runner.llm_selection.update_submitted(model_name)

        # Reset metadata
        self.meta_patch_data |= {
            "patch_type": self.patch_type,
            "api_costs": self.api_cost,
            "num_applied": 0,
            "patch_name": None,
            "patch_description": None,
            "error_attempt": "Max attempts reached without sucessful patch.", # Default error
            "novelty_attempt": novelty_attempt_index,
            "resample_attempt": None, # FIXME set value to 1 if it breaks
            "patch_attempt": 0,
            **self.llm_kwargs,
            "llm_result": None,
            "diff_summary": {},
        ...
        for patch_attempt_idx in range(self.max_patch_attempts):
            ...
            except PatchJob.PatchError as error:
    ...
    async def _try_generate_patch(self, patch_attempt_index: int) -> QueryResult:
        """
        Tries to generate a patch, raising a `PatchError` with an appropriate message if the attempt fails.
        
        :param patch_attempt_index: the index of this attempt
        :type patch_attempt_index: int
        :return llm_response: the result of the patch generation query if the query succeeds
        :rtype: QueryResult
        :raises PatchJob.PatchError: if the query to the LLM results in an empty response
        """
        ...
        if llm_response and llm_response.content:
            self.api_cost += llm_response.cost
            return llm_response
        
        # Handle empty response
        self.runner.logger.info(
            f"  PATCH ATTEMPT {patch_attempt_index + 1}/{max_patch_attempts} FAILURE. "
            f"Error: LLM response content was None."
        )
        if patch_attempt_index < max_patch_attempts - 1:
            self.user_msg = (
        ...
        raise PatchJob.PatchError(error)

    def _try_parse_and_apply_patch(self, llm_response: QueryResult, patch_attempt_index: int) -> None:
        """
        Tries to parse a patch from an LLM response and apply it to the
        parent program, raising a `PatchError` with an appropriate
        error message if the attempt fails.
        
        :param llm_response: the response of the LLM, as returned by `self._try_generate_patch`
        :type llm_response: QueryResult
        :param patch_attempt_index: the index of this patch attempt
        :type patch_attempt_index: int
        :raises PatchJob.PatchError: if the attempt fails
        """
        ...
        (
            ...
            original_str=self.parent_program.code,
            patch_str=llm_response.content,
            patch_dir=self._output_path(),
            language=self.runner.evo_config.language,
            verbose=False,
        )

        # Handle any errors
        if error_message is not None or num_applied_attempt == 0:
            error_str = error_message or "No changes applied."

            # Set prompt information for next attempt
            self.user_msg = (
        ...
        self.runner.logger.info(
            f"  PATCH ATTEMPT {patch_attempt_index + 1}/{self.max_patch_attempts} SUCCESS. "
            f"Output: {output_path_attempt}, "
            f"Patches Applied: {num_applied_attempt}."
        )

        # Only consider the diff summary for the original source file
        original_filename = f"original.{self.runner.lang_ext}"
        if original_filename in diff_summary:
            diff_summary = diff_summary[original_filename]

        # Update the relevant fields of the metadata
        self.meta_patch_data |= {
            "api_costs": self.api_cost,
            "num_applied": num_applied_attempt,
            "patch_name": patch_name,
            "patch_description": patch_description,
            "patch_attempt": patch_attempt_index + 1,
            "llm_result": llm_response.to_dict() if llm_response else None,
    ...
    async def _accept_patch_novelty(self) -> bool:
        ...
        :return should_accept: `True` if the program should be accepted, `False` otherwise.
        """

        assert self.parent_program is not None and self.current_gen != 0, "expected this job to have a parent program"

        # Get the code embedding for the evaluated code
        code_embedding, embedding_cost = self.runner.get_code_embedding(self._exec_filename())
        self.code_embedding = code_embedding
        self.embed_cost += embedding_cost

        if not code_embedding:
            self.runner.novelty_judge.log_novelty_skip_message("no embedding")
            # Accept programs with no code embedding
            return True

        novelty_judge = self.runner.novelty_judge
        if not novelty_judge.should_check_novelty(code_embedding, self.current_gen, self.parent_program, self.runner.db):
            island_manager = self.runner.db.island_manager
            if not island_manager or not hasattr(island_manager, "are_all_islands_initialized"):
                novelty_judge.log_novelty_skip_message("no island manager")
            elif not island_manager.are_all_islands_initialized():
                novelty_judge.log_novelty_skip_message("not all islands initialized yet")
            # Accept programs that shouldn't have their novelty checked
            return True
        ...
        self.novelty_cost += novelty_metadata.get("novelty_total_cost", 0.0)
        self.num_novelty_checks = novelty_metadata.get("novelty_checks_performed", 0)
        self.novelty_explanation = novelty_metadata.get("novelty_explanation", "")

        return should_accept

    def _submit_as_running_job(self) -> None:
        """
        Submits this job as a `RunningJob` in the runner's job queue.
        """

        # Add to running jobs queue
        job_id = self.runner.scheduler.submit_async(self._exec_filename(), self._results_dirname())

        # Add to running jobs queue
        running_job = RunningJob(
            job_id=job_id,
            exec_fname=self._exec_filename(),
            ...
            meta_patch_data=self.meta_patch_data,
            code_embedding=self.code_embedding,
            embed_cost=self.embed_cost,
            novelty_cost=self.novelty_cost,
        )
    ...
    def _output_path(self) -> Path:
        num_digits = math.ceil(math.log(self.runner.evo_config.num_branches_per_generation, 10))
        return Path(self.runner.results_dir).joinpath(
            f"{FOLDER_PREFIX}_{self.current_gen}_branch_{self.branch_id:0{num_digits}d}"
        )

    def _results_path(self) -> Path:
        return self._output_path().joinpath("results")

    def _results_dirname(self) -> str:
        return str(self._results_path())

    def _exec_filepath(self) -> Path:
        return self._output_path().joinpath(f"main.{self.runner.lang_ext}")

    def _exec_filename(self) -> str:
        return str(self._exec_filepath())
...
class TunaEvolutionRunner:
    """
    An evolution runner that supports the EvoTune algorithm.
    In the context of EvoTune, DPO training requires multiple generation attempts
    with the same prompt. This is incompatible with the ShinkaEvolve strategy of
    creating a different prompt for every novelty attempt.
    Note: this runner will ignore `self.evo_config.max_patch_resamples`.
    """
    ...
    async def _run_generation_0(self):
        ...
        if self.evo_config.init_program_path:
            ...
            self.logger.info(f"Initial program generated and saved to {exec_fname}")
    ...
    async def _submit_new_generation(self) -> None:
        """Submit to the queue the jobs related to a new generation."""
        current_gen = self.next_generation_to_submit
        ...
        # Get current meta-recommendations for this job
        meta_recommendations = self.meta_summarizer.get_current()
        ...
        # Sample parent/archive/top_k only once per generation
        ...
        self.logger.info(f"Edit Cycle {current_gen} -> {current_gen + 1}")

        meta_recs, _, _ = meta_recommendations

        # Sample the system and user messages only once per generation
        system_msg, user_msg, patch_type = self.prompt_sampler.sample(
            parent=parent_program,
            archive_inspirations=archive_programs,
            top_k_inspirations=top_k_programs,
            meta_recommendations=meta_recs
        ...
        async with asyncio.TaskGroup() as tg:
            for branch_id in range(self.evo_config.num_branches_per_generation):
                patch_job = PatchJob(
                    runner=self,
                    current_gen=current_gen,
                    branch_id=branch_id,
        ...
        if evaluated_code != "":
            ...
            try:
                if self.embedding is not None:
                    ...
                    self.logger.debug("=> EMBED: No embedding model configured.")
                    embedding_result = []
                    e_cost = 0.0
                code_embedding = cast(List[float], embedding_result)
            except Exception as e:
    ...
    def _restore_meta_memory(self) -> None:
        ...
        self.logger.info(f"Attempting to restore meta memory from: {meta_memory_path}")
