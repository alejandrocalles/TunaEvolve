...
from shinka.llm import LLMClient, AsyncLLMClient
...
class AsyncNoveltyJudge:
    ...
    def __init__(
        self,
        novelty_llm_client: Optional[AsyncLLMClient] = None,
        language: str = "python",
        similarity_threshold: float = 1.0,
        max_novelty_attempts: int = 3,
    ):
    ...
    async def assess_novelty_with_rejection_sampling(
        self,
        exec_fname: str,
        code_embedding: List[float],
        parent_program: Program,
            ...
            if self.novelty_llm_client is not None:
                ...
                if most_similar_program:
                    try:
                        # Read the current proposed code
                        proposed_code = Path(exec_fname).read_text(encoding="utf-8")
                        is_novel, explanation, cost = await self.check_llm_novelty(
                            proposed_code, most_similar_program
                        )
                        should_reject = not is_novel
                        novelty_cost = cost
    ...
    async def check_llm_novelty(
        ...
        try:
            response = await self.novelty_llm_client.query(
                msg=user_msg,
                system_msg=NOVELTY_SYSTEM_MSG,
                llm_kwargs=self.novelty_llm_client.get_kwargs(),
            )
